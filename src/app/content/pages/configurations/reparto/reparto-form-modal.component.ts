...
@Component({
  selector: 'app-reparto-form-modal',
  template: `  <h1 mat-dialog-title *ngIf="!data.editMode">
      {{'CONFIGURATION.NEW_REPARTO' | translate}}
  </h1>
  <h1 mat-dialog-title *ngIf="data.editMode">
      {{'CONFIGURATION.EDIT_REPARTO' | translate}}
  </h1>
  <hr>
  <div mat-dialog-content>
      <form [formGroup]="repartoForm" id="addForm" (ngSubmit)="save()">
          <div class="form-group">
              <label for="description"> {{'CONFIGURATION.REPARTO_DESCRIPTION' | translate}}:</label>
              <div class="input-group-icon input-group-icon-left">
                    <span class="input-icon input-icon-left">
                        <i class="ti-info"></i>
                    </span>
                  <input type="text" class="form-control" formControlName="description"
                         [ngClass]="{'is-invalid': submitted && repartoForm.get('description').invalid}"
                         #descInput>
              </div>
              <div *ngIf="submitted && repartoForm.get('description').invalid;" class="d-block invalid-feedback">
  ...
  <div mat-dialog-actions class="d-flex justify-content-between"
      ...
      <div>
          ...
          <button mat-flat-button color="primary"
                  [disabled]="repartoForm.get('description').disabled"
                  form="addForm"
...
export class RepartoFormModalComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<RepartoFormModalComponent>,
  ...
  ngOnInit(): void {
    this.repartoForm = new FormGroup({
      id: new FormControl(),
      description: new FormControl(null, [Validators.required])
    });
    if (this.data.editMode) {
      this.repartoForm.patchValue(this.data.reparto);
    }
  }
  ...
  save() {
    ...
    this.repartoForm.get('description').disable();
    this.addedElement = true;
    if (!this.data.editMode) { // add new preferred contact
      this.configurationsService.addNewReparto(this.repartoForm.getRawValue()).subscribe(d => {
        ...
        this.repartoForm.get('description').enable();
        this.snackBar.open(this.translate.instant('DIALOG.INTERNAL_SERVER_ERROR'), 'Ok', {duration: 5000});
      }, () => {
        this.repartoForm.get('description').enable();
        this.repartoForm.get('description').setValue(null);
        this.submitted = false;
        this.descInput.nativeElement.focus();
      });
    } else {
      this.configurationsService.editReparto(this.repartoForm.getRawValue()).subscribe(d => {
      }, (err) => {
        this.repartoForm.get('description').enable();
        this.snackBar.open(this.translate.instant('DIALOG.INTERNAL_SERVER_ERROR'), 'Ok', {duration: 5000});
      }, () => {
