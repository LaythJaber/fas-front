...
import {GoogleProduct} from "../../../../../../shared/models/sale-app/google/google-product";
...
export class ProductListComponent implements OnInit {
  ...
  /***************** Google Merchant *********************/
  addProductToGoogleMerchant(product: Product) {
    product.inLoading = true;
    const ref = this.matDialog.open(ProductMgmComponent, {
      width: 'auto',
      autoFocus: true,
      disableClose: false,
      data: {saleApp: this.data.saleApp, product: product, editMode: false},
    });
  ...
  addSelectedProductToGoogleMerchant() {
    this.productSelectedToAddInLoading = true;
    const pl: GoogleProduct[]  = [];
    for (let i =0; i<this.productToAddList.length; i++) {
      if (this.productToAddList[i] === true) {
        pl.push(this.googleMerchantService.mapProductToSaleAppProduct(this.responseProduct.data[i], this.data.saleApp.googleMerchantDomain));
      }
    }
    console.log('pl = ', pl);
    this.googleMerchantService.addSelectedProductList(this.data.saleApp.googleMerchantUserId, pl)
      .subscribe((response) => {
  ...
  addFilteredProductToGoogleMerchant() {
    console.log('start add filtering products !!!');
    this.productFilteredToAddInLoading = true;
    this.googleMerchantService.addFilteredProductList(this.request).
    subscribe((response) => {
